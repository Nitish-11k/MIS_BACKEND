...
def get_trend_data(branch_code: str = "ALL", period: str = "ALL", start_date: Optional[str] = None, end_date: Optional[str] = None):
    ...
    where_clause, params = get_date_filter_sql(period, "dw.FACT_ACCOUNT_SNAPSHOT", date_col="SNAPSHOT_DATE", start_date=start_date, end_date=end_date)
    ...
    try:
        cursor.execute(f"""
            SELECT CONVERT(VARCHAR, SNAPSHOT_DATE, 23) as dte,
                   SUM(CASE WHEN ACCOUNT_CATEGORY = 'DEPOSIT' THEN ISNULL(CURRENT_BALANCE, 0) ELSE 0 END) AS Deposits,
                   SUM(CASE WHEN ACCOUNT_CATEGORY IN ('LOAN', 'CC_OD') THEN ISNULL(OUTSTANDING, 0) ELSE 0 END) AS Loans
        ...
        for r in rows:
            data.append({
                "name": r[0],
                "Deposits": abs(r[1] or 0) / 100000,
                "Loans": abs(r[2] or 0) / 100000
            })
    except Exception as e:
...
def get_npa_trend(branch_code: str = "ALL", period: str = "ALL", start_date: Optional[str] = None, end_date: Optional[str] = None):
    ...
    where_clause, params = get_date_filter_sql(period, "dw.FACT_LOAN_RISK_DELINQUENCY", date_col="SNAPSHOT_DATE", start_date=start_date, end_date=end_date)
    ...
    try:
        cursor.execute(f"""
            SELECT CONVERT(VARCHAR, SNAPSHOT_DATE, 23) as dte,
                   SUM(CASE WHEN IS_CONFIRMED_NPA = 1 THEN ISNULL(BALANCE_OUTSTANDING, 0) ELSE 0 END) * 100.0 / NULLIF(SUM(ISNULL(BALANCE_OUTSTANDING, 0)), 0) as npa_pct
