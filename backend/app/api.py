...
def get_branch_filter_sql(branch_code, prefix="WHERE", col="BRANCH_CODE"):
    if not branch_code or branch_code == "ALL":
        return "", []
    if branch_code.startswith("REGION:"):
        region = branch_code[7:]
...
def get_date_filter_sql(period: str = None, table_name: str = "", prefix: str = "WHERE", date_col: str = "PROC_DATE", start_date: str = None, end_date: str = None):
    ...
    if period == "ALL" or not period:
        ...
        if table_name in balance_tables:
            sql = f" {prefix} CONVERT(date, {table_name}.{date_col}, 103) = (SELECT MAX(CONVERT(date, {date_col}, 103)) FROM {table_name}) "
            return sql, []
        return "", []
...
def get_notifications(branch_code: str = "ALL"):
    ...
    try:
        ...
        where_branch = ""
        params = []
        branch_sql, branch_params = get_branch_filter_sql(branch_code, "WHERE", "BRANCH_CODE")
        where_branch += branch_sql
...
def create_user(req: CreateUserRequest):
    ...
    try:
        ...
        return {"success": False, "message": "Failed to create user (Username might already exist)"}
    finally:
        conn.close()
...
def get_regions():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT DISTINCT REGIONAL_OFFICE FROM BRANCH_NETWORK WHERE REGIONAL_OFFICE IS NOT NULL")
        rows = cursor.fetchall()
...
def get_branches():
    ...
    try:
        ...
        cursor.execute("""
            SELECT DISTINCT BRANCH_CODE, BRANCH_NAME, REGIONAL_OFFICE 
            FROM BRANCH_NETWORK
        ...
        for row in rows:
            branches.append({
                ...
                "region": row[2].strip() if row[2] else ""
            })
    except Exception as e:
        import pyodbc
        if not (isinstance(e, pyodbc.Error) and len(e.args) > 0 and e.args[0] == '42S02'):
...
def get_kpi_summary(branch_code: str = "ALL", period: str = "ALL", start_date: Optional[str] = None, end_date: Optional[str] = None):
    ...
    try:
        ...
        branch_sql, branch_params = get_branch_filter_sql(branch_code, "AND" if "WHERE" in where_loan else "WHERE")
        where_loan += branch_sql
        params_loan.extend(branch_params)
        ...
        branch_sql, branch_params = get_branch_filter_sql(branch_code, "AND" if "WHERE" in where_npa else "WHERE")
...
def get_branch_network(branch_code: str = "ALL"):
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        branch_sql, branch_params = get_branch_filter_sql(branch_code, "WHERE", "BRANCH_CODE")
...
def get_loan_type_distribution(branch_code: str = "ALL", period: str = "ALL", start_date: Optional[str] = None, end_date: Optional[str] = None):
    ...
    branch_sql, branch_params = get_branch_filter_sql(branch_code, "AND" if "WHERE" in where_clause.upper() else "WHERE", "BRANCH_CODE")
    where_clause += branch_sql
    params.extend(branch_params)
    try:
        cursor.execute(f"""
        ...
        """, params)
        rows = cursor.fetchall()
        data = [{"name": r[0][:30] if r[0] else "Unknown", "raw_name": r[0] if r[0] else "Unknown", "value": float(r[1] or 0)} for r in rows]
    except:
        data = []
    conn.close()
    return data
...
def get_trend_data(branch_code: str = "ALL", period: str = "ALL", start_date: Optional[str] = None, end_date: Optional[str] = None):
    ...
    branch_sql_dep, branch_params_dep = get_branch_filter_sql(branch_code, "AND" if "WHERE" in where_dep.upper() else "WHERE", "BRANCH_CODE")
    where_dep += branch_sql_dep
    params_dep.extend(branch_params_dep)
    try:
        # Group loans by date
        cursor.execute(f"""
...
def get_account_metrics(branch_code: str = "ALL", period: str = "ALL", start_date: Optional[str] = None, end_date: Optional[str] = None):
    ...
    try:
        ...
        branch_sql, branch_params = get_branch_filter_sql(branch_code, "AND" if "WHERE" in where_sql_opened.upper() else "WHERE", "BRANCH_CODE")
        where_sql_opened += branch_sql
        params_opened.extend(branch_params)
        ...
        branch_sql, branch_params = get_branch_filter_sql(branch_code, "AND" if "WHERE" in where_sql_closed.upper() else "WHERE", "BRANCH_CODE")
        where_sql_closed += branch_sql
        params_closed.extend(branch_params)
...
def audit_exceptions(branch_code: Optional[str] = None, period: str = "ALL", start_date: Optional[str] = None, end_date: Optional[str] = None):
    try:
        ...
        branch_sql, branch_params = get_branch_filter_sql(branch_code, "AND" if "WHERE" in branch_filter else "WHERE")
        branch_filter += branch_sql
        filter_params.extend(branch_params)
